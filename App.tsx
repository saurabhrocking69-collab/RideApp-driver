...
const [hourlyTimerSec, setHourlyTimerSec]     = useState(0);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try { const r = await authFetch(`${API}/api/chat/${activeRide.id}`); const d = await r.json(); setChatMsgs(d.messages || []); lastChatCount.current = (d.messages || []).length; setUnreadChat(0); } catch (_e) {}
    };
    load();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const r = await authFetch(`${API}/api/chat/h_${activeHourlyRide.id}`);
        const d = await r.json();
  ...
  const sendHourlyChat = async (text?: string) => {
    ...
    try {
      const res = await authFetch(`${API}/api/chat/send`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ride_id: `h_${activeHourlyRide.id}`, sender: 'driver', message: msg }) });
      if (!res.ok) {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => {
      try {
        const r = await authFetch(`${API}/api/chat/${activeRide.id}`);
        const d = await r.json();
  ...
  const sendChat = async (text?: string) => {
    ...
    try { await authFetch(`${API}/api/chat/send`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ride_id: activeRide.id, sender: 'driver', message: msg }) }); const r = await authFetch(`${API}/api/chat/${activeRide.id}`); const d = await r.json(); setChatMsgs(d.messages || []); } catch (_e) {}
  };
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" automaticallyAdjustKeyboardInsets contentContainerStyle={{ paddingBottom: 130 }}>
          ...
          {false && activeRide && (
            <View style={s.tripCard}>
              ...
              {/* Trip chalu hote hi chhup jaati hai - neeche 'kitna baaki'
                  wali aa jaati hai, aur do bina-label ke number ek saath
                  (jisme bacha hua poore se bada) uljhan hi paida karte hain.
                  Ye number accept ka faisla lene ke liye tha; wo kaam ho chuka. */}
              {eta && activeRide.status !== 'started' ? <View style={{ backgroundColor: 'rgba(34,197,94,0.12)', borderRadius: 10, padding: 10, marginBottom: 10, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(34,197,94,0.3)' }}><Text style={{ color: '#22C55E', fontWeight: '700', fontSize: 13 }}>🕐 {eta}</Text></View> : null}
        ...
        {activeRide && (
          <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 160 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              {/* Trip chalu hote hi chhup jaati hai - neeche 'kitna baaki' wali
                  aa jaati hai, aur do bina-label ke number ek saath (jisme bacha
                  hua poore se bada) uljhan hi paida karte hain. Ye number accept
                  ka faisla lene ke liye tha; wo kaam ho chuka. */}
              {eta && activeRide.status !== 'started' ? (
